...
def model(x_testp):
    i=0
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i,shuffle=True,test_size=1)
    clf = make_pipeline(StandardScaler(), SVC(gamma='auto'))
    clf.fit(x_train, y_train)
   
    listclassifier.append(clf)
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+1,shuffle=True,test_size=1)
    modele_regLog = linear_model.LogisticRegression(solver = 'liblinear', multi_class = 'auto')
#training
    modele_regLog.fit(x_train, y_train)
    
    listclassifier.append(modele_regLog)
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+2,shuffle=True,test_size=1)
    classifieur = GaussianNB()
    classifieur.fit(x_train, y_train)
    
    listclassifier.append(classifieur)
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+3,shuffle=True,test_size=1)
    classifieur1= SVC(kernel='linear')
    classifieur1.fit(x_train, y_train)
    
    listclassifier.append(classifieur1)
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+4,shuffle=True,test_size=1)
...
# Train Decision Tree Classifer
    ...
    listclassifier.append(clf)
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+5,shuffle=True,test_size=1)
...
#Run model to assess accuracy
    ...
    x_train, x_test, y_train, y_test = train_test_split(X, Y,random_state=i+6,shuffle=True,test_size=1)
    classifieur = GaussianNB()
    classifieur.fit(x_train, y_train)
    listclassifier.append(classifieur)
    y_predict=[]
